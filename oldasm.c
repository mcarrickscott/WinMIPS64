...
{"cvt.w.d",F_TYPE, REG2F,   SF(F_CVT_W_D)},
...
{
    int i=0;
    char text[8];    /* all instructions are 6 chars or less */
    char *ptr=start;
    if (ptr==NULL) return (-1); 
    while (i<8 && !delimiter(*ptr)) 
    {
        text[i++]=tolower(*ptr);
        ptr++;
    }
...
/* instruction found - parse it out */
  ...
  case BRANCH:
        rt=getreg(ptr); if (rt<0) break;
        ptr=skip(ptr,','); if (ptr==NULL) break;
        rs=getreg(ptr); if (rs<0) break;
        ptr=skip(ptr,','); if (ptr==NULL) break;
        ptr=eatwhite(ptr);
        if (!getcodesym(ptr,&w)) break;    
...
/* printf("line= %d\n",lineptr);  */
    ...
    fclose(fp);
    printf("Pass 2 completed with %d errors\n",errors);
    if (errors) return 0;
